...
EXTERN_C UINT VALAPI ValueInvoke( VALUE* pval, VALUE* pthis, UINT argc, const VALUE* argv, VALUE* pretval, LPCWSTR url);
  ...
  namespace json
  {
    ...
    inline void str_copy(char* dst,const char* src, unsigned n ) { dst && src? strncpy_s(dst, strlen(dst) + 1, src,n):0; }
    inline void str_copy(wchar_t* dst,const wchar_t* src, unsigned n ) { dst && src? wcsncpy_s(dst, sizeof(dst) *(wcslen(dst) + 1), src,n):0; }
